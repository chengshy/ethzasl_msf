 ...
 */
#ifndef VISION_MEASUREMENTMANAGER_H
#define VISION_MEASUREMENTMANAGER_H
...
#include <msf_updates/vision_sensor_handler/vision_sensorhandler.h>
#include <msf_updates/vision_sensor_handler/vision_measurement.h>
#include <msf_updates/SingleVisionSensorConfig.h>

namespace msf_vision_sensor {

typedef msf_updates::SingleVisionSensorConfig Config_T;
typedef dynamic_reconfigure::Server<Config_T> ReconfigureServer;
typedef shared_ptr<ReconfigureServer> ReconfigureServerPtr;

class VisionSensorManager : public msf_core::MSF_SensorManagerROS<
    msf_updates::EKFState> {
  typedef VisionSensorHandler<VisionSensorManager> VisionSensorHandler_T;
  friend class VisionSensorHandler<VisionSensorManager> ;
 public:
  ...
  VisionSensorManager(
      ros::NodeHandle pnh = ros::NodeHandle("~/vision_sensor")) {
    imu_handler_.reset(
        new msf_core::IMUHandler_ROS<msf_updates::EKFState>(*this, "msf_core",
                                                            "imu_handler"));

    vision_handler_.reset(
        new VisionSensorHandler_T(*this, "", "vision_sensor"));
    AddHandler(vision_handler_);

    reconf_server_.reset(new ReconfigureServer(pnh));
    ReconfigureServer::CallbackType f = boost::bind(
        &VisionSensorManager::Config, this, _1, _2);
    reconf_server_->setCallback(f);
  }
  virtual ~VisionSensorManager() {
  }
 ...
 private:
  ...
  shared_ptr<VisionSensorHandler_T> vision_handler_;
  ...
  virtual void Config(Config_T &config, uint32_t level) {
    config_ = config;
    vision_handler_->SetNoises(config.vision_noise_meas_p, config.vision_noise_meas_yaw);
    vision_handler_->SetDelay(config.vision_delay);
    if ((level & msf_updates::SingleVisionSensor_INIT_FILTER)
  ...
  void Init(double scale) const {
    ...
    // vision sensor measures).
    double yawinit = config_.vision_yaw_init / 180 * M_PI;
    ...
    p_vc = vision_handler_->GetPositionMeasurement();

    MSF_INFO_STREAM(
        "initial measurement pos:[" << p_vc.transpose() << "] orientation: " << STREAMQUAT(q));

    // check if we have already input from the measurement sensor
    if (vision_handler_->ReceivedFirstMeasurement())
      MSF_WARN_STREAM(
          "No measurements received yet to initialize position - using [0 0 0]");

    ros::NodeHandle pnh("~");
    pnh.param("vision_sensor/init/p_ip/x", p_ip[0], 0.0);
    pnh.param("vision_sensor/init/p_ip/y", p_ip[1], 0.0);
    pnh.param("vision_sensor/init/p_ip/z", p_ip[2], 0.0);
...
#endif  // VISION_MEASUREMENTMANAGER_H

 ...
 */
#ifndef VISION_RANGE_MEASUREMENTMANAGER_H
#define VISION_RANGE_MEASUREMENTMANAGER_H
...
#include <msf_updates/vision_xy_sensor_handler/vision_xy_sensorhandler.h>
...
#include <msf_updates/VisionRangeSensorConfig.h>

namespace msf_vision_range_sensor {

typedef msf_updates::VisionRangeSensorConfig Config_T;
typedef dynamic_reconfigure::Server<Config_T> ReconfigureServer;
typedef shared_ptr<ReconfigureServer> ReconfigureServerPtr;

class VisionRangeSensorManager : public msf_core::MSF_SensorManagerROS<
    msf_updates::EKFState> {
  typedef msf_vision_xy_sensor::VisionXYSensorHandler<VisionRangeSensorManager> VisionXYSensorHandler_T;
  friend class msf_vision_xy_sensor::VisionXYSensorHandler<VisionRangeSensorManager> ;
 public:
  typedef msf_updates::EKFState EKFState_T;
  typedef EKFState_T::StateSequence_T StateSequence_T;
  typedef EKFState_T::StateDefinition_T StateDefinition_T;

  VisionRangeSensorManager(
      ros::NodeHandle pnh = ros::NodeHandle("~/vision_range_sensor")) {
    imu_handler_.reset(
        new msf_core::IMUHandler_ROS<msf_updates::EKFState>(*this, "msf_core",
                                                            "imu_handler"));

    vision_xy_handler_.reset(
        new VisionXYSensorHandler_T(*this, "", "vision_sensor"));
    AddHandler(vision_xy_handler_);
    ...
    ReconfigureServer::CallbackType f = boost::bind(
        &VisionRangeSensorManager::Config, this, _1, _2);
    reconf_server_->setCallback(f);
  }
  virtual ~VisionRangeSensorManager() {
  }
 ...
 private:
  ...
  shared_ptr<VisionXYSensorHandler_T> vision_xy_handler_;
  ...
  void Init(double scale) const {
    ...
    p_vc.block<2,1>(0,0) = vision_xy_handler_->GetPositionMeasurement();
    p_vc.block<1,1>(2,0) = range_handler_->GetRangeMeasurement();
...
#endif  // VISION_RANGE_MEASUREMENTMANAGER_H

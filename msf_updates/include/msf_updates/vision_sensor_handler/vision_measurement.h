 ...
 */
#ifndef VISION_MEASUREMENT_HPP_
#define VISION_MEASUREMENT_HPP_
...
/**
 * \brief A measurement as provided by a vision sensor   XYZ yaw
 */
typedef msf_core::MSF_Measurement<
    geometry_msgs::PoseStamped,
    Eigen::Matrix<double, nMeasurements, nMeasurements>, msf_updates::EKFState> VisionMeasurementBase;
struct VisionMeasurement : public VisionMeasurementBase {
 private:
  typedef VisionMeasurementBase Measurement_t;
  ...
  virtual void MakeFromSensorReadingImpl(measptr_t msg) {
    ...
    // Get measurement.
    z_p_ = Eigen::Matrix<double, 3, 1>(msg->pose.position.x, 
                                       msg->pose.position.y,
                                       msg->pose.position.z);
    ...
    R_ = (Eigen::Matrix<double, nMeasurements, 1>() << s_zp, s_zp, s_zp, s_zq).finished().asDiagonal();
 ...
 public:
  ...
  virtual ~VisionMeasurement() {
  }
  VisionMeasurement(double n_zp, double n_zq,
                      bool isabsoluteMeasurement, int sensorID, int fixedstates,
                      bool enable_mah_outlier_rejection, double mah_threshold)
      : VisionMeasurementBase(isabsoluteMeasurement, sensorID,
  ...
  virtual void Apply(shared_ptr<EKFState_T> state_nonconst_new,
    ...
    if (isabsolute_) {  // Does this measurement refer to an absolute measurement,
      ...
      r_old.block<1, 1>(3, 0) = get_yaw(qerr);
...
}  // namespace vision measeurement
}  // namespace msf_updates

#endif  // VISION_MEASUREMENT_HPP_
